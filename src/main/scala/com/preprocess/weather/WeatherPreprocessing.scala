...
class WeatherPreprocessing(private val rawWeatherData: DataFrame, private val timezoneData: DataFrame, private val airportList: DataFrame) {
  ...
  private def extractWeatherTypes(ColumnName: String): String = {
    val weatherTypesList = List("RA", "SN", "FG", "FG+", "WIND", "FZDZ", "FZRA", "MIFG", "FZFG")
    val defaultValue = "OTHER"
    if (weatherTypesList.contains(ColumnName)) {
      ColumnName
    }
    else {
      defaultValue
  ...
  private def cleanData(weatherDF: DataFrame): DataFrame = {
    ...
    val extractWeatherTypes: UserDefinedFunction = udf(extractWeatherTypes _)
    ...
    val weatherWithAverages = weatherWithAirports
      .join(dailyAverage, weatherWithAirports("AIRPORT_ID") === dailyAverage("AVERAGE_AIRPORT_ID") && weatherWithAirports("Date") === daily_average("AVERAGE_Date"), "inner")
